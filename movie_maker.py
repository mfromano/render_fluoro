...
class MakeCine(object):
    def __init__(self):
        ...
        parser.add_argument('--outputname', dest='outputname',
                            default=os.path.join(os.path.abspath('.'), 'cropped_files', 'video.mp4'))
    ...
    def load_videos(self, filenames=None):
        ...
        for filename in filenames[1:]:
            ...
            img_stack = np.append(
                img_stack, np.expand_dims(img, axis=3), axis=3)
    ...
    def upsample_frames(self, upsample_ratio=2):
        ...
        upsampled_stack = np.zeros(
            (y * upsample_ratio, x * upsample_ratio, c, z))
        for idx in range(z):
            upsampled_stack[:, :, :, idx] = cv2.resize(np.squeeze(self.img_stack[:, :, :, idx]), (
    ...
    def write_frames(self, img_stack=None, prefix=''):
        ...
        for idx in range(img_stack.shape[-1]):
            ...
            img.save('{}/{}{}.png'.format(self.outdir,
    ...
    def compute_flow(self):
        ...
        for idx in range(self.img_stack.shape[-1] - 1):
            ...
            curr_frame = cv2.cvtColor(
                curr_frame.astype(np.uint8), cv2.COLOR_RGB2GRAY)
            next_frame = np.squeeze(self.img_stack[:, :, :, idx + 1])
            next_frame = cv2.cvtColor(
                next_frame.astype(np.uint8), cv2.COLOR_RGB2GRAY)
            flow = cv2.calcOpticalFlowFarneback(
                curr_frame, next_frame, flow=flow, pyr_scale=0.8, levels=15, winsize=5, iterations=10, poly_n=5, poly_sigma=0, flags=10)
...
if __name__ == '__main__':
    ...
    cine.write_video()
    cine.compute_flow()
    cine.write_video(cine.flow, 'flow_video.mp4')
